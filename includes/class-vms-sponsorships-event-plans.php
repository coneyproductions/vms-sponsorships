...
class VMS_Sponsorships_Event_Plans {
    ...
    public function save_event_plan_meta($post_id, $post) {
        ...
        $nonce = sanitize_text_field($request['vms_sponsorships_event_plan_meta_nonce'] ?? '');
        if ($nonce === '' || !wp_verify_nonce($nonce, 'vms_sponsorships_event_plan_meta')) {
            return;
        }
        ...
        $tier = sanitize_key($request['vms_sponsorship_value_tier'] ?? 'standard');
        if (!array_key_exists($tier, $this->value_tiers())) {
            $tier = 'standard';
        }

        update_post_meta($post_id, '_vms_sponsorship_value_tier', $tier);
        update_post_meta($post_id, '_vms_expected_attendance', absint($request['vms_expected_attendance'] ?? 0));
        update_post_meta($post_id, '_vms_sponsorship_price_multiplier', max(0, (float) ($request['vms_sponsorship_price_multiplier'] ?? 1)));
    }
